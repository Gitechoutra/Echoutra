    ...
    for s in STOCKS:
        try:
            ...
            if existing:
                ...
                existing.open_price          = Decimal(str(s.get("open_price", s["current_price"])))
                existing.day_high            = Decimal(str(s.get("day_high",   s["current_price"])))
                existing.day_low             = Decimal(str(s.get("day_low",    s["current_price"])))
                existing.price_change        = chg
                existing.price_change_percent= chg_p
                existing.volume              = s.get("volume",   0)
                existing.avg_volume          = s.get("avg_volume",0)
                existing.market_cap          = Decimal(str(s["market_cap"]))  if s.get("market_cap")   else None
                existing.pe_ratio            = Decimal(str(s["pe_ratio"]))    if s.get("pe_ratio")     else None
                existing.eps                 = Decimal(str(s["eps"]))         if s.get("eps")          else None
                existing.dividend_yield      = Decimal(str(s["dividend_yield"])) if s.get("dividend_yield") is not None else None
                existing.beta                = Decimal(str(s["beta"]))        if s.get("beta")         else None
                existing.week_52_high        = Decimal(str(s["week_52_high"])) if s.get("week_52_high") else None
                existing.week_52_low         = Decimal(str(s["week_52_low"]))  if s.get("week_52_low")  else None
                ...
                db.session.commit()
                updated += 1
                print(f"  [UPDATED] {s['ticker']} ({s['currency']})")
            else:
                stock = Stocks()
                ...
                stock.current_price       = curr
                stock.previous_close      = prev
                stock.open_price          = Decimal(str(s.get("open_price", s["current_price"])))
                stock.day_high            = Decimal(str(s.get("day_high",   s["current_price"])))
                stock.day_low             = Decimal(str(s.get("day_low",    s["current_price"])))
                stock.price_change        = chg
                stock.price_change_percent= chg_p
                stock.volume              = s.get("volume",    0)
                stock.avg_volume          = s.get("avg_volume", 0)
                ...
                stock.week_52_high        = Decimal(str(s["week_52_high"])) if s.get("week_52_high") else None
                stock.week_52_low         = Decimal(str(s["week_52_low"]))  if s.get("week_52_low")  else None
